...
import { InventoryProduct } from './our-product.types';
...
export class OurProductComponent implements OnInit {
  ...
  constructor(
    ...
    private _apiService: OurProductService,
  ...
  private _prepareItemsListForm(itemsListObj: any = {}): void {
    this.discountsForm = this._formBuilder.group({
      ourproductid   : [itemsListObj?._id || ''],
      ourproductname : [itemsListObj?.ourproductname || '', [Validators.required]],
      description    : [itemsListObj?.description || '', [Validators.required]],
      image          : [itemsListObj?.image || '', [Validators.required]],
      socialmedia    : [itemsListObj?.socialmedia || '', [Validators.required]],
      links          : [itemsListObj?.links || '', [Validators.required]],
    });
  }
